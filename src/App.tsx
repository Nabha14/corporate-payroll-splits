...
export default function App() {
  ...
  const deployContractAction = async () => {
    ...
    try {
      ...
      setContractDeployed(true);
      logTransaction(result.txId, 'CONTRACT DEPLOYMENT SUBMITTED', '—', 'payroll deployed on Midnight Preview at ' + result.contractAddress);
    } catch (err) {
      ...
      setDeployStep(prev => {
        if (prev < deploySteps.length - 1) {
          ...
          setTimeout(() => {
            ...
            logTransaction('0xdep1...88bb', 'CONTRACT DEPLOYED', '-15.50 tNIGHT', 'Deployed payroll.compact contract onto Preview');
          }, 800);
          return prev;
        }
      });
