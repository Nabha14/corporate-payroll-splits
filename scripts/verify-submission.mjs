...
const required = ['README.md', 'PROPOSAL.md', 'TESTING.md', 'src/test/payroll.test.ts', 'deployment.json', 'SECURITY.md', 'OPERATIONS.md', 'src/runtimeConfig.ts', 'src/test/runtime-config.test.ts'];
...
const [readme, proposal, testing, suite, deploymentText] = await Promise.all([
  ...
  readFile('src/test/payroll.test.ts', 'utf8'),
...
if (!testing.includes('src/test/payroll.test.ts')) throw new Error('TESTING.md must identify the executable suite.');
...
if (deployment.contract !== 'payroll' && deployment.contractName !== 'payroll') {
  throw new Error('Deployment manifest does not identify the payroll contract.');
}
...
console.log('Payroll assurance check: proposal, src/test/payroll.test.ts, and Preview deployment evidence verified.');
